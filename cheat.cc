...
namespace open_spiel
{
  namespace cheat
  {
    ...
    std::string CheatState::ToString() const
    {
      ...
      if (!passed_cards_[0].empty())
        absl::StrAppend(&rv, FormatPass());
    ...
    std::string CheatState::InformationStateString(Player player) const
    {
      ...
      if (!passed_cards_[player].empty())
        absl::StrAppend(&rv, FormatPass(player));
      if (num_cards_played_ > 0)
        absl::StrAppend(&rv, FormatPlay(), FormatPoints());
      return rv;
    }
                                            ...
                                            absl::Span<float> values) const
    {
      ...
      for (int i = 0; i < kNumCards; ++i)
        if (player_hand_[i] == player)
          ptr[i] = 1;
      ptr += kNumCards;
      // Cards claimed
      for (int i = 0; i < kNumCards; ++i)
        if (card_claimed_[i] == player)
          ptr[i] = 1;
      ptr += kNumCards;
      // Cards seen
      for (int i = 0; i < kNumCards; ++i)
        if (card_seen_[i] == player)
          ptr[i] = 1;
      ptr += kNumCards;
      // Action history
    ...
    std::vector<std::pair<Action, double>> CheatState::ChanceOutcomes() const
    {
      ...
      for (int card = 0; card < kNumCards; ++card)
      {
        if (!player_hand_[card].has_value())
